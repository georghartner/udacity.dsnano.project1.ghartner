...
class Line:
    ...
    def process_message(self, message):
        ...
        if self.topic_name_pattern == "com.udacity.dsnano.ghartner.project1.stationOrder": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif self.topic_name_pattern.startswith("com.udacity.dsnano.ghartner.project1.arrivals."): # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif self.topic_name_pattern == "TURNSTILE_SUMMARY": # Set the conditional to the KSQL Turnstile Summary Topic
